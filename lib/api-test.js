...
class APITest {
	...
	async assert(rule, defaultEndpoint) {
		...
		const endpoint = rule.endpoint || defaultEndpoint || 'foo';

		const dispatcher = new Dispatcher({ endpoint });
		dispatcher.api = this.prepareApi(rule, endpoint);
